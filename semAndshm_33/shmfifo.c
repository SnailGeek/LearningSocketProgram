...
//key用来创创建shmid, sem_empty, sem_full, sem_mutex 的ID
shmfifo_t* shmfifo_init(int key ,int blksize, int blocks)
{
	...
	int size = sizeof(shmhead_t) + blocks * blksize;
	if(shmid == -1){
		fifo->shmid = shmget(key, size, IPC_CREAT | 0666);
		if(fifo->shmid == -1)
			ERR_EXIT("shmget");
		
		fifo->p_shm = (shmhead_t*)shmat(fifo->shmid, NULL, 0); //连接共享内存
...
void shmfifo_get(shmfifo_t* fifo, char *buf)
{
	...
	memcpy(buf, fifo->p_payload + fifo->p_shm->blksize * fifo->p_shm->wr_index, fifo->p_shm->blksize);
	fifo->p_shm->wr_index = (fifo->p_shm->wr_index + 1) % fifo->p_shm->blocks;
